		...
		public virtual unsafe double ErrorCorrection {
			...
			get {
				...
				try {
					...
					if (GetType () == ThresholdType)
						return JNIEnv.CallDoubleMethod  (Handle, id_getErrorCorrection);
					else
						return JNIEnv.CallNonvirtualDoubleMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "getErrorCorrection", "()D"));
				} finally {
				}
			}
			...
			set {
				...
				try {
					...
					if (GetType () == ThresholdType)
						JNIEnv.CallVoidMethod  (Handle, id_setErrorCorrection_D, __args);
					else
						JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "setErrorCorrection", "(D)V"), __args);
				} finally {
				}
			}
		}
