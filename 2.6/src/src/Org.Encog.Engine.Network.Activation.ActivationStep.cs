		...
		public virtual unsafe double Center {
			...
			get {
				...
				try {
					...
					if (GetType () == ThresholdType)
						return JNIEnv.CallDoubleMethod  (Handle, id_getCenter);
					else
						return JNIEnv.CallNonvirtualDoubleMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "getCenter", "()D"));
				} finally {
				}
			}
			...
			set {
				...
				try {
					...
					if (GetType () == ThresholdType)
						JNIEnv.CallVoidMethod  (Handle, id_setCenter_D, __args);
					else
						JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "setCenter", "(D)V"), __args);
				} finally {
				}
			}
		}
