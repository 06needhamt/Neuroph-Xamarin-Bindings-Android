	...
	public partial class Layer : global::Java.Lang.Object, global::Java.IO.ISerializable {
		...
		protected IList<Org.Neuroph.Core.Neuron> Neurons {
			get {
				...
				return JavaArray<global::Org.Neuroph.Core.Neuron>.FromJniHandle (JNIEnv.GetObjectField (Handle, neurons_jfieldId), JniHandleOwnership.TransferLocalRef);
			}
			set {
				if (neurons_jfieldId == IntPtr.Zero)
					neurons_jfieldId = JNIEnv.GetFieldID (class_ref, "neurons", "[Lorg/neuroph/core/Neuron;");
				IntPtr native_value = JavaArray<global::Org.Neuroph.Core.Neuron>.ToLocalJniHandle (value);
				try {
					JNIEnv.SetField (Handle, neurons_jfieldId, native_value);
				} finally {
		...
		public virtual unsafe void RemoveAllNeurons ()
		{
			...
			try {
				...
				if (GetType () == ThresholdType)
					...
					JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "removeAllNeurons", "()V"));
			} finally {
			}
		}
