...
#pragma warning restore 0169
		...
		public virtual unsafe void TestNeuralNetwork ()
		{
			...
			try {
				...
				if (GetType () == ThresholdType)
					JNIEnv.CallVoidMethod  (Handle, id_testNeuralNetwork);
				else
					JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "testNeuralNetwork", "()V"));
			} finally {
			}
		}
...
#pragma warning restore 0169
		...
		public virtual unsafe void Train ()
		{
			...
			try {
				...
				if (GetType () == ThresholdType)
					JNIEnv.CallVoidMethod  (Handle, id_train);
				else
					JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "train", "()V"));
			} finally {
			}
		}
