...
namespace Org.Neuroph.Nnet.Learning {
	...
	public partial class BackPropagation : global::Org.Neuroph.Nnet.Learning.LMS {
		...
		protected virtual unsafe void CalculateErrorAndUpdateHiddenNeurons ()
		{
			...
			try {
				...
				if (GetType () == ThresholdType)
					JNIEnv.CallVoidMethod  (Handle, id_calculateErrorAndUpdateHiddenNeurons);
				else
					JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "calculateErrorAndUpdateHiddenNeurons", "()V"));
			} finally {
			}
		}

		static Delegate cb_calculateErrorAndUpdateOutputNeurons_arrayD;
#pragma warning disable 0169
		static Delegate GetCalculateErrorAndUpdateOutputNeurons_arrayDHandler ()
		{
			if (cb_calculateErrorAndUpdateOutputNeurons_arrayD == null)
				cb_calculateErrorAndUpdateOutputNeurons_arrayD = JNINativeWrapper.CreateDelegate ((Action<IntPtr, IntPtr, IntPtr>) n_CalculateErrorAndUpdateOutputNeurons_arrayD);
			return cb_calculateErrorAndUpdateOutputNeurons_arrayD;
		}

		static void n_CalculateErrorAndUpdateOutputNeurons_arrayD (IntPtr jnienv, IntPtr native__this, IntPtr native_p0)
		{
			global::Org.Neuroph.Nnet.Learning.BackPropagation __this = global::Java.Lang.Object.GetObject<global::Org.Neuroph.Nnet.Learning.BackPropagation> (jnienv, native__this, JniHandleOwnership.DoNotTransfer);
			double[] p0 = (double[]) JNIEnv.GetArray (native_p0, JniHandleOwnership.DoNotTransfer, typeof (double));
			__this.CalculateErrorAndUpdateOutputNeurons (p0);
			if (p0 != null)
				JNIEnv.CopyArray (p0, native_p0);
		}
#pragma warning restore 0169

		static IntPtr id_calculateErrorAndUpdateOutputNeurons_arrayD;
		// Metadata.xml XPath method reference: path="/api/package[@name='org.neuroph.nnet.learning']/class[@name='BackPropagation']/method[@name='calculateErrorAndUpdateOutputNeurons' and count(parameter)=1 and parameter[1][@type='double[]']]"
		[Register ("calculateErrorAndUpdateOutputNeurons", "([D)V", "GetCalculateErrorAndUpdateOutputNeurons_arrayDHandler")]
		protected virtual unsafe void CalculateErrorAndUpdateOutputNeurons (double[] p0)
		{
			if (id_calculateErrorAndUpdateOutputNeurons_arrayD == IntPtr.Zero)
				id_calculateErrorAndUpdateOutputNeurons_arrayD = JNIEnv.GetMethodID (class_ref, "calculateErrorAndUpdateOutputNeurons", "([D)V");
			...
			try {
				...
				if (GetType () == ThresholdType)
					JNIEnv.CallVoidMethod  (Handle, id_calculateErrorAndUpdateOutputNeurons_arrayD, __args);
				else
					JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "calculateErrorAndUpdateOutputNeurons", "([D)V"), __args);
			} finally {
				if (p0 != null) {
...
#pragma warning restore 0169
		...
		protected virtual unsafe double CalculateHiddenNeuronError (global::Org.Neuroph.Core.Neuron p0)
		{
			...
			try {
				...
				double __ret;
				if (GetType () == ThresholdType)
					__ret = JNIEnv.CallDoubleMethod  (Handle, id_calculateHiddenNeuronError_Lorg_neuroph_core_Neuron_, __args);
				else
					__ret = JNIEnv.CallNonvirtualDoubleMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "calculateHiddenNeuronError", "(Lorg/neuroph/core/Neuron;)D"), __args);
				return __ret;
			} finally {
			}
		}
