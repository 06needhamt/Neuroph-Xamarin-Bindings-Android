		...
		public virtual unsafe double Sigma {
			...
			get {
				...
				try {
					...
					if (GetType () == ThresholdType)
						return JNIEnv.CallDoubleMethod  (Handle, id_getSigma);
					else
						return JNIEnv.CallNonvirtualDoubleMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "getSigma", "()D"));
				} finally {
				}
			}
			...
			set {
				...
				try {
					...
					if (GetType () == ThresholdType)
						JNIEnv.CallVoidMethod  (Handle, id_setSigma_D, __args);
					else
						JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "setSigma", "(D)V"), __args);
				} finally {
				}
			}
		}
