		...
		protected virtual unsafe void AdjustOutputNeurons (double[] p0)
		{
			...
			try {
				...
				if (GetType () == ThresholdType)
					JNIEnv.CallVoidMethod  (Handle, id_adjustOutputNeurons_arrayD, __args);
				else
					JNIEnv.CallNonvirtualVoidMethod  (Handle, ThresholdClass, JNIEnv.GetMethodID (ThresholdClass, "adjustOutputNeurons", "([D)V"), __args);
			} finally {
				if (p0 != null) {
